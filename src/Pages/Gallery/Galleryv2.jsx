...
const Galleryv2 = () => {
  ...
  const [isHiddenSecondPanelImage, setIsHiddenSecondPanelImage] =
    useState(false);
  // for right animation
  ...
  const handleNext = () => {
    ...
    setTimeout(() => {
      setBoxes((prev) => {
        ...
        // Right rotate by one: remove the last element and prepend it to the start
        const updatedBoxes = [
          prev[prev.length - 1],
          ...prev.slice(0, prev.length - 1),
        ];
        // Optionally update carouselImage based on the new first element
        setCarouselImage(updatedBoxes[0]?.img || "");
        return updatedBoxes;
      });
      setIsAnimatingRight(false);
      setTimeout(() => {
        setIsHiddenSecondPanelImage(false);
      }, 600);
    }, carouselAnimationDuration); // Matches the same animation duration
    setTimeout(() => {
      setIsHiddenSecondPanelImage(true);
    }, 300);
  };
  ...
  return (
    <div className="flex flex-col h-[112vh] md:h-[112vh] lg:h-screen py-4 px-4  gap-y-4 md:gap-y-0 lg:gap-y-0">
      ...
      {/* Gallery section */}
      <motion.div className="flex-[100%]  mg:flex-[100%] lg:flex-[65%] flex flex-col sm:flex-col md:flex-col lg:flex-row gap-x-4 gap-y-8 relative ">
        {/* first panel */}
        <motion.div
          className={`overflow-hidden px-4  flex flex-col ${
            isAnimating || isAnimatingRight || isHiddenSecondPanelImage
              ? "absolute"
              : "hidden"
          } w-[921px] h-[600px]  rounded-4xl ${
            isHiddenSecondPanelImage && isAnimatingRight!=true ? "z-10" : "-z-10"
          }`}
          initial={{ opacity: 1, scale: 0.8, x: 0, y: 0 }}
          animate={{
            opacity: isAnimating || isAnimatingRight ? [1] : 1,
            scale: isAnimating || isAnimatingRight ? [1] : 1,
            x: isAnimating || isAnimatingRight ? -10 : 0,
            y: isAnimating || isAnimatingRight ? 10 : -10,
          }}
          transition={{
        ...
        >
          ...
          <motion.div
            ...
            initial={{ scale: 1 }}
            animate={{
              scale: isAnimating || isAnimatingRight ? [0.9, 1.9] : 1.1,
            }}
        ...
        {/* <motion.div
          className={`overflow-hidden px-4 border ${
            isAnimating ? "absolute" : "absolute"
          } w-[800px] h-[600px]  rounded-4xl`}
          initial={{ opacity: 0, scale: 0.8, x: 0,y:0 }}
          animate={{
            opacity: isAnimating ? [1] : 0,
            scale: isAnimating ? [1] : 1,
            x: isAnimating ? -10 : 100,
            y: isAnimating ? 0 : -100,
          }}
          transition={{
        ...
        {/* second panel */}
        {/* left image section second panel */}
        <motion.div
          className={`flex-[50%] rounded-4xl flex flex-col overflow-hidden px-4 relative ${
            isHiddenSecondPanelImage &&isAnimatingRight!=true ? "-z-10" : "z-10"
          }`}
          initial={{ opacity: 1, scale: 0.8, x: -112 }}
          animate={{
            opacity: isAnimating || isAnimatingRight ? [] : 1,
            scale: isAnimating || isAnimatingRight ? [1.2, 1.05, 1.1] : 1,
            x: isAnimating  ? -900 : isAnimatingRight?100: 0,
            y: isAnimating ? 900 : 0,
          }}
          transition={{
            opacity: {
              duration: isAnimating ? 0.6 : isAnimatingRight ? 0.6 : 0.3,
              ease: "easeOut",
            },
            scale: {
              duration: isAnimating ? 0.6 : isAnimatingRight ? 0.6 : 0.3,
        ...
        >
          ...
          <motion.div
            ...
            initial={{ scale: 1 }}
            animate={{
              scale: isAnimating ? [1.4, 1.8] : 1.1,
              opacity: isAnimating || isHiddenSecondPanelImage ? 0 : 1,
            }}
            transition={{ duration: 0.4, ease: "easeOut" }}
          />
          {/* Utility */}
        ...
        <motion.div
          className="px-0 py-0 flex-[50%] flex-col relative z-[40] gap-y-2"
        ...
        >
          ...
          <div className="flex-[90%] hidden sm:hidden md:flex lg:flex flex-row  px-2 gap-x-6 justify-between py-14 overflow-hidden">
            {/* Gallery Frame 2 */}
            <GalleryCard
              isAnimating={isAnimating || isAnimatingRight}
              box={boxes[1]}
              scaleImg={true}
              scale={1.2}
              x={isAnimating ? -124 : isAnimatingRight ? 112 : -124}
            />

            {/* Gallery Frame 3 */}
            <GalleryCard isAnimating={isAnimating ||isAnimatingRight} box={boxes[2]} x={isAnimating ? -112 : isAnimatingRight ? 152 : -112} />
            {/* Gallery Frame 4 */}
            <GalleryCard isAnimating={isAnimating || isAnimatingRight} box={boxes[3]} x={isAnimating ? -124 : isAnimatingRight ? 162 : -124}      scaleImg={true}/>
          </div>
        </motion.div>
      </motion.div>
