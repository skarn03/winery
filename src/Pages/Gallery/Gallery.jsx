...
const Gallery = () => {
  ...
  const handleNextRight = () => {
    ...
    setTimeout(() => {
      ...
      setBoxesSmall((prev) => [...prev.slice(1), prev[0]]);
  ...
  return (
    <div className="flex flex-col h-[112vh] md:h-[112vh] lg:h-screen py-4 px-4  gap-y-4 md:gap-y-0 lg:gap-y-0">
      ...
      >
        {/* left image section */}
        <motion.div className="flex-[50%] rounded-4xl flex flex-col overflow-hidden px-4 "
          whileHover={{ scale: 1 }}
          whileTap={{ scale: 0.94 }}
        ...
        >
          ...
          {/* Information  */}
          <div className="flex-[25%] text-white px-2 flex flex-col gap-y-0 sm:gap-y-0 md:gap-y-2 lg:gap-y-2">
            <div><p className="font-bold text-[#C2C2C2] text-[12px] sm:text-[12px] md:text-[16px] lg:text-[20px]">Location</p></div>
            <div> <p className="font-bold text-[#fff] text-[20px] sm:text-[20px] md:text-[28px] lg:text-[32px]">Vineyard</p></div>
            <div> <p className="font-bold text-[#fff] text-[12px] sm:text-[12px] md:text-[20px] lg:text-[24px] mb-2">Wine made from the best Grapes</p></div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex-[90%] hidden sm:hidden md:flex lg:flex flex-row overflow-hidden px-2 gap-x-5 justify-between py-4">
            {boxes.map((box, index) => {
          ...
          {/* Images section for smaller screen  */}
          <div className="grid sm:grid md:hidden lg:hidden grid-cols-2 gap-5 px-1   py-4 ">
            {boxesSmall.map((box, index) => {
              const scaleFactor = isAnimating ? 1.2 - index * 0.25 : 1;
              const scale = Math.max(scaleFactor, 0.8);
              return (
                <motion.div
                  key={box.id}
                  className=" "
                ...
                >
                  <div className="flex flex-col justify-center flex-[90%] gap-y-3">
                    <img
                      src={box.img}
                      alt="Gallery Item"
                      className="w-[200px] h-[180px] rounded-4xl"
                    />
                    {/* Information */}
                    <div className="flex flex-col gap-y-2 px-2">
                      <div>
                      ...
                      <div>
                        <p className="max-w-[150px] text-[#1C272C] text-[10px] sm:text-[10px] md:text-[12px] lg:text-[12px] ">
                          {box.description}
                        </p>
                      </div>
                    </div>
